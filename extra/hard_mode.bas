Sub setup_v_mode_shortcuts()
  ...
  Application.OnKey "h", "'visual_left """ & anchor_row & """, " & anchor_col & " '"
  Application.OnKey "{BS}", "'visual_left """ & anchor_row & """, " & anchor_col & " '"
  Application.OnKey "j", "'visual_down """ & anchor_row & """, " & anchor_col & " '"
  Application.OnKey "k", "simple_visual_up"
  Application.OnKey "l", "'visual_right """ & anchor_row & """, " & anchor_col & " '"
  Application.OnKey " ", "'visual_right """ & anchor_row & """, " & anchor_col & " '"
  ' etc. 
End Sub

' visual up
Public Sub visual_up(anchor_row As Long, anchor_col As Long)
  ...
  If top_row > 1 And top_row < anchor_row Then
...
' visual down
Public Sub visual_down(anchor_row As Long, anchor_col As Long)
  Dim anchor_range As Range: Set anchor_range = Cells(anchor_row, anchor_col)
  Dim top_row As Long: top_row = Selection.row
  Dim bottom_row As Long: bottom_row = top_row + Selection.Rows.Count - 1
  Dim left_col As Long: left_col = Selection.Column
  Dim right_col As Long: right_col = Selection.Columns.Count + left_col - 1
  Dim end_range As Range
  If bottom_row < 1048576 And bottom_row > anchor_row Then
    If left_col < anchor_col Then
      Set end_range = Cells(bottom_row + 1, left_col)
    Else
      Set end_range = Cells(bottom_row + 1, right_col)
...
' visual left
Public Sub visual_left(anchor_row As Long, anchor_col As Long)
  Dim anchor_range As Range: Set anchor_range = Cells(anchor_row, anchor_col)
  Dim top_row As Long: top_row = Selection.row
  Dim bottom_row As Long: bottom_row = top_row + Selection.Rows.Count - 1
  Dim left_col As Long: left_col = Selection.Column
  Dim right_col As Long: right_col = Selection.Columns.Count + left_col - 1
  Dim end_range As Range
  If left_col > 1 And left_col < anchor_col Then
...
' visual right
Public Sub visual_right(anchor_row As Long, anchor_col As Long)
  Dim anchor_range As Range: Set anchor_range = Cells(anchor_row, anchor_col)
  Dim top_row As Long: top_row = Selection.row
  Dim bottom_row As Long: bottom_row = top_row + Selection.Rows.Count - 1
  Dim left_col As Long: left_col = Selection.Column
  Dim right_col As Long: right_col = Selection.Columns.Count + left_col - 1
  Dim end_range As Range
  If right_col < 116384 And right_col > anchor_col Then
    If top_row < anchor_row Then
      Set end_range = Cells(top_row, right_col + 1)
    Else
      Set end_range = Cells(bottom_row, right_col + 1)
